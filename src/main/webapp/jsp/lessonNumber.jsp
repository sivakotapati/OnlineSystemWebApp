...
<script type="text/javascript">
	...
	function buildLessonList(response) {
		...
		for (let i = 0; i < response.content.lessonAndQuizStatus.length; i++) {
			...
			li += "<li>";
			if (lessonId == 1) {
				li += "<a href='lesson.jsp?lessonNumber="
						+ response.content.lessonAndQuizStatus[i].lessonId
				li += "'>"
			} else {
				li += "<a href='lesson.jsp?lessonNumber="
						+ response.content.lessonAndQuizStatus[i].lessonId
			...
			li += response.content.lessonAndQuizStatus[i].lessonId + "!"
					+ "</a>";
			li += "&emsp;";
			...
			li += "&emsp;";
			li += "&emsp;";
			li += "<a class='btn btn-primary' href='quiz.jsp?lessonNumber="
					+ response.content.lessonAndQuizStatus[i].lessonId + "'>";
			li += "Assessment ";
	...
	document.getElementById('lesson').setAttribute('href',
			'lesson.jsp?lessonNumber=' + lessonId);
</script>